...
# txt files written by Bruker can be parsed by readBrukerTxt
	...
	if ((format == "csv") | (format == "csv2") | (format == "Btxt")) {
	...
	if (!alignTMS) { # default, assumes file all have some npoints
		...
		for (i in 1:length(files)) {
			...
			if (debug) cat("Processing file: ", files[i], "\n")
			spectra$data[i,] <- temp[,2]
			}
				
	} # end of alignTMS = FALSE
	
	if (alignTMS) { # These files differ in number of data points

		if (!format == "Btxt") stop("alignTMS only works for format = 'Btxt'")
		cat("\n>>> Locating and aligning spectra on TMS/TSP signal\n")
		cat("\n>>> You've been warned -- this is experimental!\n")
		...
		for (i in 1:length(files)) {
			temp <- readBrukerTxt(files[i], debug = debug)
			if (dim(temp)[1] > maxNP) {
				maxNP <- dim(temp)[1]
				best <- i
				}
