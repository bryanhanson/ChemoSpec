...
# This script reads a series of files that share a common frequency axis.
...
# DX files can also be parsed (see readJDX for limitations)
	...
	if ((format == "csv") | (format == "csv2")) files.noext <- substr(basename(files), 1, nchar(basename(files)) - 4)
	...
	for (i in 1:length(files)) {
		...
		if (format == "dx") temp <- readJDX(files[i])
