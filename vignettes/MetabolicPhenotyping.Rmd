...
@Blaise2021 have published a detailed workflow for metabolomic phenotyping.  This vignette will follow their analysis closely to demonstrate how to carry it out using `ChemoSpec`.  The data is composed of 139 ^13^C HR-MAS SS-NMR spectra [@Blaise2007] of the model organism *Caenorhabditis elegans*.  There are two genotypes, wild type and a mutant, and worms from two life stages.
...
p <- plotScores(Worms, c_pca, pcs = 2:3, ellipse = "rob", leg.loc = list(x = 0.85, y = 0.3), tol = 0.01)
p
```
...
p <- p + coord_cartesian(ylim = c(0.045, 0.06))
p
```
