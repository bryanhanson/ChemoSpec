...
vignette: >
    ...
    %\VignetteIndexEntry{Testing Plot Combinations (for developers)}
...
tol = c("none", 0.02)
# combine options
opts <- expand.grid(loads, go, stringsAsFactors = FALSE,tol)
...
for (i in 1:nrow(opts)) {
  ...
  Title <- paste("Loads(s) = ", paste(opts$loads[[i]], collapse = ", "), opts$tol[i])
...
tol = c("none", 0.02)
# combine options
opts <- expand.grid(pc, go, stringsAsFactors = FALSE,tol)
...
tol = c("none", 0.02)
# combine options
opts <- expand.grid(pcs, usesym, plt, go, stringsAsFactors = FALSE,tol)
