...
Also included is a data set \texttt{gasNIR} which is the \texttt{gasoline} data set that is included with \texttt{R}, converted to \texttt{Spectra} format.  During the conversion, the octane values were transformed from numeric values to membership in L, M, or H classes by dividing the octane values into terciles.  The \texttt{gasNIR} data set does not separate well (so far) by any statistical method I have employed, so I use it mainly for testing.  
...
Example~\ref{colors} shows these colors. Finally, the current color scheme of a \texttt{Spectra} object may be determined using \texttt{sumGroups} or changed using \texttt{conColScheme}. 
...
plotScores(G.only, G.only.pca, title = "Two Genotypes/Control Conditions",
ellipse = "rob")
@
\end{center}
\end{figure}
